...
func WithRLS(c *gin.Context, dbConn *sql.DB, fn func(tx *sql.Tx) error) error {
	...
	_, err = tx.ExecContext(c.Request.Context(), "SELECT set_config('app.current_user_id', $1, true)", uid)
	if err != nil {
		return err
	}
	_, err = tx.ExecContext(c.Request.Context(), "SELECT set_config('app.current_user_roles', $1, true)", roles)
	if err != nil {
		return err
	}
